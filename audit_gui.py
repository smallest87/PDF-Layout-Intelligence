...
# --- WIDGET RULER DENGAN OFFSET SCROLL ---
class RulerWidget(QWidget):
    ...
    def update_ruler(self, zoom, offset):
        """Memperbarui skala penggaris berdasarkan zoom dan posisi scroll."""
        self.zoom = zoom
        self.offset = offset
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Hanya gambar area yang terlihat di layar
    ...
    def mousePressEvent(self, event):
        """Memicu pembuatan guide baru dari penggaris."""
...
class AuditGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PPU Audit System - Ultimate Edition")
        ...
        self._is_paginating = False # Guard untuk mencegah loop saat pindah halaman
        
        # Data Guides (Persistent)
        self.v_guides = [] 
        self.h_guides_dict = {} 
    ...
    def init_ui(self):
        ...
        # PDF Panel (Grid + Controls di bawah)
        ...
        # Sinyal untuk Ruler dan Auto-Pagination
        ...
        # Panel Navigasi di Bawah Page Preview
        nav_control = QHBoxLayout()
        btns = [
        ...
        for text, func in btns:
            ...
            nav_control.addWidget(btn)
        nav_control.insertSpacing(3, 20) # Beri jarak antara zoom dan nav
        nav_control.addStretch()
        pdf_panel.addLayout(nav_control)
    ...
    def sync_rulers(self):
        """Update posisi penggaris mengikuti scroll."""
    ...
    def handle_scroll_pagination(self, value):
        ...
        if v_bar.maximum() > 0 and value >= v_bar.maximum():
            if self.current_page < len(self.doc) - 1:
                ...
                self.current_page += 1
                self.render_overlay()
                v_bar.setValue(0) # Mulai dari atas halaman baru
        ...
        # Pindah ke Halaman Sebelumnya
            ...
            self.current_page -= 1
            self.render_overlay()
            v_bar.setValue(v_bar.maximum()) # Mulai dari bawah halaman sebelumnya
            self._is_paginating = False
    ...
    def prev_page(self):
        if self.current_page > 0: self.current_page -= 1; self.render_overlay()
    def next_page(self):
        if self.doc and self.current_page < len(self.doc)-1: self.current_page += 1; self.render_overlay()
    ...
    def show_edit_dialog(self, g_type, index, current_val):
        ...
        new_val, ok = QInputDialog.getDouble(self, "Edit Koordinat", label, current_val, 0, 5000, 2)
        if ok:
            if g_type == 'h': self.h_guides_dict[self.current_page][index] = new_val
            else: self.v_guides[index] = new_val
    ...
    def start_new_guide(self, g_type, pos_pts):
        if g_type == 'v':
            ...
            if self.current_page not in self.h_guides_dict: self.h_guides_dict[self.current_page] = []
    ...
    def on_pdf_mouse_press(self, event):
        ...
        for i, pts in enumerate(curr_h):
            if abs(event.pos().y() - (pts * self.zoom_factor)) < 15:
                self.active_drag = {'type': 'h', 'index': i}; return
        for i, pts in enumerate(self.v_guides):
            if abs(event.pos().x() - (pts * self.zoom_factor)) < 15:
                self.active_drag = {'type': 'v', 'index': i}; return

    def on_pdf_mouse_move(self, event):
        if self.active_drag:
            new_pts = (event.pos().y() if self.active_drag['type'] == 'h' else event.pos().x()) / self.zoom_factor
            if self.active_drag['type'] == 'h': self.h_guides_dict[self.current_page][self.active_drag['index']] = new_pts
            else: self.v_guides[self.active_drag['index']] = new_pts
            self.render_overlay()
    ...
    def render_overlay(self):
        ...
        try:
            ...
            # Gunakan .stride (fix pitched error)
                ...
                for idx, row in self.df[mask].iterrows():
                    try:
                        ...
                        if idx == self.selected_row_index:
                            painter.setPen(QPen(QColor(255, 255, 0), 2)); painter.setBrush(QColor(255, 255, 0, 80))
                            target_x, target_y = rx, ry
                        else:
                            painter.setPen(QPen(QColor(0, 0, 255, 50), 1)); painter.setBrush(Qt.BrushStyle.NoBrush)
            ...
            for pts in self.v_guides:
                sx = int(pts * zoom); painter.drawLine(sx, 0, sx, pix.height)
            for pts in self.h_guides_dict.get(self.current_page, []):
                sy = int(pts * zoom); painter.drawLine(0, sy, pix.width, sy)
            painter.end()
            self.pdf_label.setPixmap(pixmap); self.sync_rulers()
            # Auto-scroll follow highlighted row
    ...
    def select_pdf(self):
        ...
        if path:
            ...
            self.load_guides(); self.render_overlay()
    ...
    def select_csv(self):
        ...
        if path:
            ...
            self.load_table_data(); self.render_overlay()

    def load_table_data(self):
        self._is_loading = True; self.table.setRowCount(len(self.df))
        cols = ["text", "unsur", "sistematika", "x0", "x1", "top", "bottom"]
        for i, row in self.df.iterrows():
            for j, col_name in enumerate(cols): self.table.setItem(i, j, QTableWidgetItem(str(row.get(col_name, ''))))
        self._is_loading = False
    ...
    def delete_selected_row(self):
        ...
        if row >= 0 and QMessageBox.question(self, 'Hapus', f"Hapus baris {row+1}?", 
            ...
            self._is_loading = True; self.df = self.df.drop(self.df.index[row]).reset_index(drop=True)
            self.table.removeRow(row); self.df.to_csv(self.csv_path, index=False, sep=';', quoting=csv.QUOTE_ALL, quotechar='"')
            self._is_loading = False; self.render_overlay()

    def on_selection_changed(self, curr_row, curr_col, prev_row, prev_col):
        if curr_row < 0 or self.df is None: return
        self.selected_row_index = curr_row
        if 'page' in self.df.columns:
            new_pg = int(self.df.iloc[curr_row]['page']) - 1
            if new_pg != self.current_page: self.current_page = new_pg; self.render_overlay()

if __name__ == "__main__":
    app = QApplication(sys.argv); window = AuditGUI(); window.showMaximized(); sys.exit(app.exec())