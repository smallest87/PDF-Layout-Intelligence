...
class MasterAggregator:
    ...
    def process_pembukaan(self):
        ...
        diktum_parts = [m_diktum.group(i).strip() for i in range(1, 4)] if m_diktum else [diktum_raw]
    ...
    def process_batang_tubuh(self):
        ...
        def finalize(p_list, kategori="Pokok"):
            is_def = (kategori == "Ketentuan Umum")
            for p in p_list:
                clean = re.sub(rf"^\s*Pasal\s+{p['nomor']}\s*", "", p['teks'], flags=re.IGNORECASE).strip()
                p['teks'] = self._parse_ayat(clean, is_definisi=is_def)
    ...
    def process_penutup(self):
        ...
        return {"perintah_pengundangan": m_perintah.group(1).strip() if m_perintah else "NONE",
            "pengesahan": {"tempat": m_sah.group(1).strip() if m_sah else "NONE", "tanggal": m_sah.group(2).strip() if m_sah else "NONE", "nama_jabatan": m_sah.group(3).strip() if m_sah else "NONE", "nama_pejabat": m_sah.group(5).strip() if m_sah else "NONE"},
            "pengundangan": {"tempat": m_und.group(1).strip() if m_und else "NONE", "tanggal": m_und.group(2).strip() if m_und else "NONE", "nama_jabatan": m_und.group(3).strip() if m_und else "NONE", "nama_pejabat": m_und.group(5).strip() if m_und else "NONE"},
            "publikasi": [m_pub.group(1).strip(), m_pub.group(2).strip()] if m_pub else []}

    def run_all(self):
        return {"A_JUDUL": self.process_judul(), "B_PEMBUKAAN": self.process_pembukaan(), "C_BATANG_TUBUH": self.process_batang_tubuh(), "D_PENUTUP": self.process_penutup()}